...
public class AuthenticationController {
    ...
    public String processRegistrationForm(@ModelAttribute @Valid RegistrationFormDTO registrationFormDTO, Errors errors, HttpServletRequest request, Model model) {
        ...
        User existingUser = (User) userRepository.findByUsername(registrationFormDTO.getUsername());
    ...
    @PostMapping("/login")
    public String processRegistrationForm(@ModelAttribute @Valid LoginFormDTO loginForm, Errors errors, HttpServletRequest request, Model model) {
        if (errors.hasErrors()) {
            model.addAttribute("title", "Log In");
            return "login";
        }

        User theUser = (User) userRepository.findByUsername(loginForm.getUsername());

        if (theUser == null) {
            errors.rejectValue("username", "username.invalid", "The username does not exist");
            model.addAttribute("title", "Log In");
            return "login";
        }
